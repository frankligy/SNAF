...
'''
the fastq files were downloaded from dbGap, again, we follow the tutorial to get bam files (STAR) and the count matrix, we 
provided the count matrix in synapse for the following analysis.
'''
...
df = pd.read_csv('count_matrix_van_allen_melanoma.txt',sep='\t')
...
burden_output,quantiles = snaf.survival_analysis(burden,survival,n=2,stratification_plot='result/stage0_stratify.pdf',survival_plot='result/stage0_survival.pdf',survival_duration='overall_survival',survival_event='dead')
...
# survival correlation and biomarkers
freq = 0.20
